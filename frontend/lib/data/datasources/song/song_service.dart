...
class SongService {
  ...
  Future<Map<String, dynamic>> updateSongTitle(String token, String songId, String newTitle) async {
    try {
      if (kDebugMode) print('Updating song title for songId: $songId to $newTitle with token: $token');
      final response = await _dio.put(
        '/songs/$songId',
        data: {'title': newTitle},
        options: Options(
          ...
          validateStatus: (status) => true, // Allow all statuses for debugging
        ),
      );
      if (kDebugMode) print('Response status: ${response.statusCode}, Data: ${response.data}');
      if (response.statusCode == 200) {
      ...
      if (kDebugMode) print('Update song error: $e');
      if (e is DioException && kDebugMode) print('Dio error details: ${e.response?.data}');
  ...
  Future<void> deleteSong(String token, String songId) async {
    try {
      if (kDebugMode) print('Deleting song with songId: $songId with token: $token');
      ...
      if (kDebugMode) print('Delete song error: $e');
      if (e is DioException && kDebugMode) print('Dio error details: ${e.response?.data}');
